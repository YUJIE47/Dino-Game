...
def show_score(winSur, score):
    font = pygame.font.SysFont("simhei", 56)
    txt = font.render(str(int(score)), True, (100, 100, 100))
    winSur.blit(txt, (500, 260))
...
def resetPage(score):
    ...
    # 載入按鈕圖片
    background = pygame.image.load(cfg.IMAGES['GAMEOVERBG']).convert_alpha()
    surBtnNormal = pygame.image.load(cfg.IMAGES['Reset']).convert_alpha()
    surBtnMove = pygame.image.load(cfg.IMAGES['Reset']).convert_alpha()
    surBtnDown = pygame.image.load(cfg.IMAGES['Reset']).convert_alpha()
    ...
    # 建立按鈕
    btn1 = Button(500, 450, "", surBtnNormal, surBtnMove, surBtnDown, btnCallBack,btnFont,(255,0,0))
